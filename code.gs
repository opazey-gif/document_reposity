...
// ==========================================
// 1. SETUP ENVIRONMENT (เจาะจงสำหรับ SARABAN)
// ==========================================
function setupEnvironment() {
  // สร้างโฟลเดอร์สำหรับงานสารบรรณโดยเฉพาะ
  const rootFolder = DriveApp.createFolder("TIJ_Saraban_PDF_Repo");
  const stagingFolder = rootFolder.createFolder("Staging_Area");
  const activeFolder = rootFolder.createFolder("Active_Files");
  
  // สร้างฐานข้อมูลสำหรับงานสารบรรณโดยเฉพาะ
  const ss = SpreadsheetApp.create("TIJ_Saraban_Database");
  ...
  // Config ประเภทเอกสารสารบรรณ
  sheetConfig.appendRow(["Document_Type", "-", "คำสั่ง (Order)"]);
  sheetConfig.appendRow(["Document_Type", "-", "ประกาศ (Announcement)"]);
  sheetConfig.appendRow(["Document_Type", "-", "ระเบียบ (Regulation)"]);
  sheetConfig.appendRow(["Document_Type", "-", "บันทึกข้อความ (Memo)"]);
  ...
  return `✅ Setup E-Document (Saraban) Complete!\nSpreadsheet ID: ${ss.getId()}`;
}
...
function getConfigData() {
  const props = PropertiesService.getScriptProperties();
  const dbId = props.getProperty('DB_SHEET_ID');
  if(!dbId) throw new Error("System not initialized. Please run setupEnvironment first.");
...
function uploadFile(payload) {
  ...
  try {
    ...
    // ข้ามการเช็ค Hash Duplicate ถ้าเป็นโหมดออฟไลน์
    if (!payload.file_hash.startsWith("OFFLINE-")) {
      for (let i = 1; i < data.length; i++) {
    ...
    let fileId = "OFFLINE";
    let initialStatus = "ACTIVE"; // Metadata Only สามารถ Active และแสดงผลได้ทันที
    let aiStatus = "N/A"; // ไม่มีไฟล์จริงให้หุ่นยนต์อ่าน

    // กรณีปกติที่มีการอัปโหลดไฟล์เข้ามา
    if (payload.base64Data && payload.base64Data !== "") {
      ...
      aiStatus = "PENDING";
      logAuditEvent("UPLOAD", fileId, `Uploaded file: ${payload.file_name}`);
    } else {
      logAuditEvent("UPLOAD_METADATA", "OFFLINE", `Registered offline metadata: ${payload.subject}`);
    }
    ...
    txSheet.appendRow([
      ...
      payload.file_hash, initialStatus, fileId, new Date().toISOString(), aiStatus
    ]);

    return { success: true, requestId: reqId, message: "ลงทะเบียนข้อมูลสำเร็จ" };
  ...
  try {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (row[statusColIndex] === 'QUEUED') {
        const fileId = row[21];  
        const year = row[3] && row[3] !== "-" ? new Date(row[3]).getFullYear().toString() : new Date().getFullYear().toString();
        try {
...
function getDashboardStats() {
  ...
  const ss = SpreadsheetApp.openById(dbId);
  const data = ss.getSheetByName('Table_Transactions').getDataRange().getValues();
  let stats = { totalFiles: 0, totalPages: 0, totalSizeMB: 0, byDept: {}, bySecurity: {}, byType: {}, byPdpa: {} };
  ...
  for(let i = 1; i < data.length; i++) {
    ...
    let type = row[9] || 'Unknown'; stats.byType[type] = (stats.byType[type] || 0) + 1;
    let pdpa = row[11] || 'Unknown'; stats.byPdpa[pdpa] = (stats.byPdpa[pdpa] || 0) + 1;
  }
...
function updateDocument(payload) {
  ...
  try {
    ...
    txSheet.getRange(rowIndex, 4).setValue(payload.doc_date || "-");
    txSheet.getRange(rowIndex, 5).setValue(payload.effective_date || "-"); 
    txSheet.getRange(rowIndex, 6).setValue(payload.expiry_date || "-");
    txSheet.getRange(rowIndex, 8).setValue(payload.department); 
    txSheet.getRange(rowIndex, 9).setValue(payload.unit || "-");
    txSheet.getRange(rowIndex, 10).setValue(payload.document_type || "-"); 
    txSheet.getRange(rowIndex, 11).setValue(payload.security_class);
    txSheet.getRange(rowIndex, 12).setValue(payload.pdpa_flag); 
    txSheet.getRange(rowIndex, 13).setValue(payload.data_owner); 
    txSheet.getRange(rowIndex, 15).setValue(payload.project_context || "-"); 
    txSheet.getRange(rowIndex, 16).setValue(payload.tags || "-");
